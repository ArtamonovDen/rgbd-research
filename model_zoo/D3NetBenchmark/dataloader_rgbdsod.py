...
class RgbdSodDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.if_memory:
            return self.transform(self.samples[index].copy()) if self.transform is not None else self.samples[index].copy()
        else:
            return self.transform(self.get_sample(index)) if self.transform is not None else self.get_sample(index)
