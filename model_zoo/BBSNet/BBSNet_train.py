...
from bbs_utils import clip_gradient, adjust_lr
...
# load data
print('load data...')
train_loader = get_loader(image_root, gt_root, depth_root,
                          batchsize=opt.batchsize, trainsize=opt.trainsize, num_workers=2)
test_loader = test_dataset(
    test_image_root, test_gt_root, test_depth_root, opt.trainsize)
total_step = len(train_loader)

logging.basicConfig(stream=sys.stdout,
                    format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]', level=logging.INFO, datefmt='%Y-%m-%d %I:%M:%S %p')
    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                ...
                wandb.log({'loss1': loss1.data, 'loss2': loss2.data, 'loss': loss})

                writer.add_scalar('Loss', loss.data, global_step=step)
                grid_image = make_grid(
                    images[0].clone().cpu().data, 1, normalize=True)
                writer.add_image('RGB', grid_image, step)
                wandb.log({'rgb': [wandb.Image(grid_image)]})

                grid_image = make_grid(
                    gts[0].clone().cpu().data, 1, normalize=True)
                writer.add_image('Ground_truth', grid_image, step)
                wandb.log({'gt': [wandb.Image(grid_image)]})
                ...
                writer.add_image('s1', torch.tensor(res),
                                 step, dataformats='HW')
                wandb.log({'s1': [wandb.Image(torch.tensor(res))]})
                ...
                writer.add_image('s2', torch.tensor(res),
                                 step, dataformats='HW')
                wandb.log({'s2': [wandb.Image(torch.tensor(res))]})

        loss_all /= epoch_step
        logging.info(
            '#TRAIN#:Epoch [{:03d}/{:03d}], Loss_AVG: {:.4f}'.format(epoch, opt.epoch, loss_all))
        wandb.log({'avg-loss-epoch': loss_all})
